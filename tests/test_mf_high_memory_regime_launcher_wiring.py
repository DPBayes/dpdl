...
LAUNCHER = (
    ...
    / "dpdl-experiments"
    / "experiments"
    / "52-mf-high-memory-regime"
    / "scripts"
    / "run_and_resume.sh"
)
LR_ONLY_CONFIG = DPDL_DIR / "conf" / "optuna_hypers-mf-high-memory-cifar10-vgg-lr-only.conf"
LR_BANDS_CONFIG = DPDL_DIR / "conf" / "optuna_hypers-mf-high-memory-cifar10-vgg-lr-bands.conf"
...
def _run_launcher(
    *,
    ...
    if override_log_paths:
        env.update(
            {
                "LOG_DIR_BASE": "outputs/test-mf-high-memory-regime",
                "CMD_LOG_DIR_BASE": "experiments/52-mf-high-memory-regime/data",
            }
        )
    if extra_env:
...
def test_high_memory_cifar_optuna_configs_are_exact() -> None:
...
def test_launcher_emits_standard_opacus_dpsgd_row() -> None:
    ...
    assert "MF high-memory regime launcher" in stdout
    ...
    assert "--optuna-config conf/optuna_hypers-mf-high-memory-cifar10-vgg-lr-only.conf" in stdout
    ...
    assert "--no-pretrained" in stdout
...
def test_launcher_emits_optimized_bandwidth_bsr_row() -> None:
    ...
    assert "--optuna-config conf/optuna_hypers-mf-high-memory-cifar10-vgg-lr-bands.conf" in stdout
...
def test_launcher_skips_completed_rows_from_runtime_file(tmp_path: Path) -> None:
    ...
    experiment_name = (
        "52-mf-highmem-cifar10-bnb-vgg-net-dpsgd-eps0p25-ep20-"
        "bs512-pbs512-clip10-sub1p0-trials25-seed42"
    )
...
def test_launcher_defaults_match_run8_scratch_path_contract(tmp_path: Path) -> None:
    ...
    (
        dpdl_dir
        / "conf"
        / "optuna_hypers-mf-high-memory-cifar10-vgg-lr-only.conf"
    ).write_text("learning_rate:\n  min: 1e-4\n  max: 0.1\n")
    (
        dpdl_dir
        / "conf"
        / "optuna_hypers-mf-high-memory-cifar10-vgg-lr-bands.conf"
    ).write_text("learning_rate:\n  min: 1e-4\n  max: 0.1\n")
    ...
    host_default = (
        ...
        "experiments/52-mf-high-memory-regime/data"
    )
    command_default = (
