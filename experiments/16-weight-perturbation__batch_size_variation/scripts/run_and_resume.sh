#!/bin/bash
####################################################################
### Experiment: Pretrained weight perturbation (batch size varation)
####################################################################
#
set -euo pipefail

# Base configurations
EXPERIMENT_BASE="16-weight-perturbation__batch_size_variation"
    ...
    do
        ...
        do
            ...
            do
                ...
                do
                    EXPERIMENT_NAME="${model}_${clean_dataset_name}_Subset${subset_size}_Epsilon${rounded_epsilon}_BatchSize${batch_size}_WeightNoiseLevel${weight_noise_level}"
